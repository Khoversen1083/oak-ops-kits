...
- Has a matching lesson (or 1 to 3 lessons) in `revenue-playbook/`,
  `lead-acquisition/`, or another playbook folder
...
8. Add a row in `oak-ops-kits/README.md` and (if applicable) link the
   matching playbook lesson to this kit.
...
- `grep -irn '\$3,\?000\|\$3K' .` returns nothing (no program price).
  Note for this kit: $1,000 to $2,500 setup fee and $300 to $600
  retainer are the TAUGHT student pricing, those are fine and required
  content. The banlist is only the $3K Oak Ops program price.
