...
The shape every kit in this repo follows. This file mirrors the spec in
`kits/cold-email/KIT-ANATOMY.md`. Fork the `cold-email/` folder, rename,
swap the topic-specific resources, and you have a new kit that behaves
the same way.
...
1. **Step 0, Setup Interview.** Check for `intake-filled.md`. If
   ...
   answers, confirm.
2. **Step 1, calibration or research.** Read the kit's first resource
   file, gather what is needed for this run.
...
- Has a matching lesson (or 1 to 3 lessons) in the playbook
- Can be run in 30 to 45 minutes for the first time
...
6. Rewrite the example files in `examples/` with a fake niche.
7. Run the validation checks (see below) before committing.
8. Add a row in `oak-ops-kits/README.md` and link any matching lesson.
